...
def lyricsfunction(artist, song):
    ...
    if not os.path.isfile(f"/home/{uname}/Bard/{artist}: {song}"):
        try:
            ...
            lyrics = html.find("div", class_="col-xs-12 col-lg-8 text-center")
            if lyrics is not None:
                lyrics = lyrics.find_all("div")[5]
                lyrics = lyrics.text
        ...
        if (key == curses.KEY_UP or key == ord("k")) and current_row > 0:
            ...
            if current_row == 0:
                ...
                while True:
                    ...
                    if (
                        keypress == curses.KEY_UP or keypress == ord("k")
                    ) and current > 0:
                        current -= 1
                    elif (
                        keypress == curses.KEY_DOWN or keypress == ord("j")
                    ) and current < len(tableau) - 1:
                        current += 1
                ...
                while True:
                    print_menu(
                        ...
                        current,
                        confirm,
                        "Are you sure you want to remove ALL saved lyrics?",
                    )
                    keypress = stdscr.getch()
                    if (
                        keypress == curses.KEY_UP or keypress == ord("k")
            ...
            # Exitting the curses menu
                ...
                while True:
                    print_menu(
                        stdscr, current, confirm, "Are you sure you want to exit?"
                    )
                    keypress = stdscr.getch()
                    if (
                        keypress == curses.KEY_UP or keypress == ord("k")
